...
def train():
    ...
    df.to_csv("FedCons_results.csv", index=False)
    sets = parse_opt()
    ...
    for rnd in range(1, sets.rounds + 1):
        ...
        for agent_id in tqdm(range(sets.num_agents)):
            # print(agent_id)
            update, diceloss, celoss, cosloss = agents[agent_id].train_local(global_model, lam)
        ...
        if best_dice < avg_valid_dice:
            ...
            torch.save(global_model.state_dict(), "FedCons_model_5%.pth")
        ...
        ndf.to_csv("FedCons_results.csv", mode="a", index=False, header=False)
...
def test():
    ...
    global_model.load_state_dict(torch.load("FedCons_model_5%.pth"))
    global_model = global_model.to(device)
