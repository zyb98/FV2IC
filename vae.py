...
class VAE(nn.Module):
    ...
    def sample(self, device='cuda'):
        # mean_value = np.load("mean_value.npy")
        # std_value = np.load("std_value.npy")
        # mean_value = np.reshape(mean_value, (1, 128))
        # std_value = np.reshape(std_value, (1, 128))
        # mean_value = torch.tensor(mean_value, dtype=torch.float32).to(device)
        # std_value = torch.tensor(std_value, dtype=torch.float32).to(device)
        z = torch.randn(1, self.latent_dim).to(device)
        # z = z * std_value + mean_value
        x = self.decoder_projection(z)
        x = torch.reshape(x, (-1, *self.decoder_input_chw))
        decoded = self.decoder(x)
        return decoded
