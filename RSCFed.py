...
class Aggregation():
    ...
    def aggregate_update(self, global_model, agent_updates_dict):
        ...
        aggregate_updates = 0
        aggregated_updates1 = 0
        aggregated_updates2 = 0
        aggregated_updates3 = 0
        ...
        rnd_select = [i for i in range(10)]
        random.shuffle(rnd_select)
        new_updates1 = {}
        for j in range(0, 3):
            new_updates1[rnd_select[j]] = agent_updates_dict[rnd_select[j]]
        new_updates2 = {}
        for j in range(3, 6):
            new_updates2[rnd_select[j]] = agent_updates_dict[rnd_select[j]]
        new_updates3 = {}
        for j in range(6, 10):
            new_updates3[rnd_select[j]] = agent_updates_dict[rnd_select[j]]
        
        aggregated_updates1 = self.agg_avg(new_updates1)
        aggregated_updates2 = self.agg_avg(new_updates2)
        aggregated_updates3 = self.agg_avg(new_updates3)

        aggregated_updates = 0.3 * aggregated_updates1 + 0.3 * aggregated_updates2 + 0.4 * aggregated_updates3
...
class Agent():
    ...
    def train_local(self, global_model, agent_updates_dict, lam):
        ...
        for p in range(10):
            para = initial_global_model_params 
        ...
        for epoch in range(self.args.epoch):
            for i, (data, label) in enumerate(self.train_loader):
                ...
                res = torch.zeros(20, 4, 1, 224, 224).to(torch.device("cuda:7"))
                ...
                loss.backward()
                optimizer.step()
                for l in range(len(self.model_bank)):
                    self.model_bank[l]  = 0.5 * self.model_bank[l] + 0.5 * parameters_to_vector(global_model.parameters()).detach()
...
def train():
    ...
    df.to_csv("RSCFed_results.csv", index=False)
    sets = parse_opt()
    ...
    for rnd in range(1, sets.rounds + 1):
        ...
        for agent_id in tqdm(range(sets.num_agents)):
            # print(agent_id)
            update = agents[agent_id].train_local(global_model, model_bank_dict, lam)
        ...
        if best_dice < avg_valid_dice:
            ...
            torch.save(global_model.state_dict(), "RSCFed_model_5%.pth")
        ...
        ndf.to_csv("RSCFed_results.csv", mode="a", index=False, header=False)
...
def test():
    ...
    global_model.load_state_dict(torch.load("RSCFed_model_5%.pth"))
    global_model = global_model.to(device)
