...
class Agent():
    def __init__(self, id, args):
        self.id = id
        self.args = args
        # self.dataset = utils.ACDCDataset("new_upper/train_data_" + str(id))
        self.dataset = utils.ACDCDataset("center_label"+str(id+1)+"_5%"+"/data")
        # self.train_loader = utils.get_train_data_loader(args.batch_size, "new_upper/train_data_" + str(id))
        self.train_loader = utils.get_train_data_loader(args.batch_size, "center_label"+str(id+1)+"_5%"+"/data")
        ...
        self.cos_dataset = utils.VAEDataset("center"+str(id+1)+"/data")
        # self.cos_loader = utils.get_generate_data_loader(20, "center"+str(id)+"/train_data_" + str(id))
        self.cos_loader = utils.get_generate_data_loader(20, "center"+str(id+1)+"/data")
    ...
    def train_local(self, global_model, lam):
        ...
        tmp_model = UNet2D(1, 4, final_sigmoid=False).to(torch.device("cuda:6"))
        ...
        for i in range(len(unlabel)):
            ...
            pre1, _, _= torch.utils.checkpoint.checkpoint(self.seg_model, unlabel_data, use_reentrant=False)
            # pre1 = self.seg_model(unlabel_data)
            pre2, z, res2 = global_model(unlabel_data)
            pre3 = self.seg_model.get_res(z, res2)
            avg_pre = (pre1 + pre2 + pre3) / 3
            _, pseudo = torch.max(avg_pre, 1)
        ...
        for epoch in range(self.args.epoch):
            for i, (data, label) in enumerate(self.train_loader):
                ...
                out_put, _, _ = global_model(aug_data)
...
def train():
    ...
    test_data = utils.get_train_data_loader(sets.batch_size, 'new_mm_valid')
    ...
    for rnd in range(1, sets.rounds + 1):
        ...
        with torch.no_grad():
            ...
            for j, (inputs, labels) in enumerate(test_data):
                ...
                outputs, _, _ = global_model(inputs)
        ...
        if best_dice < avg_valid_dice:
            ...
            torch.save(global_model.state_dict(), "Fedtrinet_model_5%.pth")
...
def test():
    ...
    global_model.load_state_dict(torch.load("Fedtrinet_model_5%.pth"))
    global_model = global_model.to(device)
    test_data = utils.get_train_data_loader(4, 'new_mm_test')
