...
class Agent():
    def __init__(self, id, args):
        ...
        self.dataset = utils.ACDCDataset("center_label"+str(id+1)+"/data")
        # self.train_loader = utils.get_train_data_loader(args.batch_size, "new_upper/train_data_" + str(id))
        self.train_loader = utils.get_train_data_loader(args.batch_size, "center_label"+str(id+1)+"/data")
    ...
    def train_local(self, global_model, lam):
        ...
        tmp_model = UNet2D(1, 4, final_sigmoid=False).to(torch.device("cuda:0"))
...
def train():
    ...
    for rnd in range(1, sets.rounds + 1):
        ...
        if best_dice < avg_valid_dice:
            ...
            torch.save(global_model.state_dict(), "FedLabel_model_20%.pth")
...
def test():
    ...
    global_model.load_state_dict(torch.load("FedLabel_model_20%.pth"))
    global_model = global_model.to(device)
