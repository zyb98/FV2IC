    ...
    def train_local(self, global_model, agent_updates_dict, lam):
        ...
        for epoch in range(self.args.epoch):
            for i, (data, label) in enumerate(self.train_loader):
                ...
                # aug_data = unlabel_data + torch.normal(0, 5, (20, 1, 1, 224, 224)).to(torch.device("cuda:0"))
                # aug_data = torch.clip(aug_data, 0, 255)
                transform_data = unlabel_data.view(20, 1, 224, 224)
                aug_data, _, _, _ = generate_model(transform_data)
...
def train():
    ...
    for rnd in range(1, sets.rounds + 1):
        ...
        for agent_id in tqdm(range(sets.num_agents)):
            # print(agent_id)
            update, diceloss, celoss, cosloss = agents[agent_id].train_local(global_model, model_bank_dict, lam)
        ...
        plt_dice = np.array(get_dice_loss[0])
        plt_ce = np.array(get_ce_loss[0])
        plt_cos = np.array(get_cos_loss[0])
        # plt_recons = np.array(recons_seg_loss[0])
        np.save("plt_dice.npy", plt_dice)
        np.save("plt_ce.npy", plt_ce)
        np.save("plt_cos.npy", plt_cos)
        ...
        if best_dice < avg_valid_dice:
            ...
            torch.save(global_model.state_dict(), "F2CMT_model_5%.pth")
        ...
        ndf.to_csv("mean_teacher_results.csv", mode="a", index=False, header=False)
...
def test():
    ...
    global_model.load_state_dict(torch.load("F2CMT_model_5%.pth"))
    global_model = global_model.to(device)
